...
6. Successful results are cached to `.cache/pr-{number}.json`, keyed by PR number
   + head SHA, so reloading without new commits skips Ollama entirely.
...
`zod`. No database — flat JSON cache on disk, no auth, API routes call GitHub
and Ollama server-side so the token never reaches the client.
